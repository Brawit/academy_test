...
class QuestsControllerTest < ActionDispatch::IntegrationTest
  ...
  # POST /quests Turbo Stream failure (empty name)
  ...
  test "should create quest (JSON)" do
    assert_difference("Quest.count", 1) do
      post quests_url, params: { quest: { name: "JSON Quest" } }, as: :json
    end
  ...
  # PATCH /quests/:id JSON
  test "should update quest (JSON)" do
    patch quest_url(@quest),
          params: { quest: { name: "Updated JSON" } },
          as: :json
    assert_response :ok
    @quest.reload
    json = JSON.parse(response.body)
